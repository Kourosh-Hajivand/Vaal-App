import SerialPortAPI from "react-native-serial-port-api";
import { Buffer } from "buffer";

/**
 * RadarLogic - A class to handle HLK-LD2410C communication.
 * This separates the hardware logic from the UI.
 */
class RadarLogic {
    constructor() {
        this.serialPort = null;
        this.subscription = null;
        this.buffer = Buffer.alloc(0);
        this.isConnected = false;
        this.isConnecting = false; // ✅ Track connection attempts

        // Callbacks to update the UI
        this.onDataUpdate = null; // Called when new sensor data arrives
        this.onLog = null; // Called for debugging logs
        this.onConfigRead = null; // Called when config is read successfully

        // Throttling control - Real-time mode (50ms = ~20 updates per second)
        this.lastUpdateTime = 0;
        this.updateInterval = 50; // Real-time updates (50ms = ~20 updates/sec)

        // 📊 Statistics & Monitoring
        this.stats = {
            packetsReceived: 0,
            configPackets: 0,
            dataPackets: 0,
            errors: 0,
            lastPacketTime: 0,
            connectionTime: 0,
            reconnectCount: 0,
            bufferOverflows: 0,
        };
    }

    /**
     * Connects to the serial port and starts the initialization sequence.
     * ✅ با race condition protection
     */
    async connect(path = '/dev/ttyS1', baudRate = 115200) {
        try {
            // ⛔ اگر در حال اتصال هستیم یا قبلاً متصل شدیم، دوباره تلاش نکن
            if (this.isConnected || this.isConnecting) {
                return;
            }

            this.isConnecting = true;
            this.log(`Connecting to ${path} at ${baudRate}...`);
            
            // باز کردن port
            this.serialPort = await SerialPortAPI.open(path, { baudRate });
            
            if (!this.serialPort) {
                throw new Error('Serial port is null');
            }

            // ✅ Mark as connected
            this.isConnected = true;
            this.isConnecting = false;
            this.stats.connectionTime = Date.now();
            this.stats.reconnectCount += 1;
            this.log(`Port Opened. Initializing... (Reconnect #${this.stats.reconnectCount})`);

            // Start listening to incoming bytes
            this.subscription = this.serialPort.onReceived(this.handleIncomingData.bind(this));

            // 🚀 STARTUP SEQUENCE: Initialize sensor
            // استفاده از Promise برای بهتر کنترل کردن timing
            await new Promise(resolve => setTimeout(resolve, 500));
            this.sendCommand("FF000100"); // Enable Config
            
            await new Promise(resolve => setTimeout(resolve, 500));
            this.sendCommand("6100"); // Read Config
            
            await new Promise(resolve => setTimeout(resolve, 500));
            this.sendCommand("FE00"); // End Config
            
        } catch (error) {
            const errorMessage = error?.message || error?.toString() || "Unknown error";
            this.stats.errors += 1;
            this.log(`Connection Error: ${errorMessage} (Total errors: ${this.stats.errors})`, "error");
            this.isConnected = false;
            this.isConnecting = false;
            // Throw error تا hook متوجه بشه
            throw error;
        }
    }

    disconnect() {
        // ✅ Clean disconnect با proper cleanup
        if (this.subscription) {
            this.subscription.remove();
            this.subscription = null;
        }
        if (this.serialPort) {
            // Note: Some libraries don't support close(), but we clean up references
            this.serialPort = null;
        }
        this.isConnected = false;
        this.isConnecting = false;
        this.buffer = Buffer.alloc(0);
        this.log("Disconnected.");
    }

    /**
     * Sends a HEX command string to the module.
     * Wraps it in the protocol header/footer.
     */
    sendCommand(hexBody) {
        if (!this.serialPort) return;

        // Construct packet: Header(FD FC FB FA) + Len + Body + Footer(04 03 02 01)
        const header = Buffer.from([0xfd, 0xfc, 0xfb, 0xfa]);
        const footer = Buffer.from([0x04, 0x03, 0x02, 0x01]);
        const body = Buffer.from(hexBody, "hex");
        const length = Buffer.alloc(2);
        length.writeUInt16LE(body.length); // Little Endian length

        const packet = Buffer.concat([header, length, body, footer]);
        this.serialPort.send(packet.toString("hex"));
        this.log(`CMD Sent: ${hexBody}`, "info");
    }

    /**
     * Handles raw incoming bytes, appends to buffer, and parses packets.
     * ✅ با Error Handling و Buffer Management بهبود یافته
     */
    handleIncomingData(buff) {
        try {
            // Append new data to the existing buffer
            this.buffer = Buffer.concat([this.buffer, Buffer.from(buff, "hex")]);

            // 🛡️ محافظت در برابر Buffer Overflow
            const MAX_BUFFER_SIZE = 2048; // 2KB max
            if (this.buffer.length > MAX_BUFFER_SIZE) {
                this.stats.bufferOverflows += 1;
                this.log(`⚠️ Buffer overflow #${this.stats.bufferOverflows} detected (${this.buffer.length} bytes), clearing...`, 'warning');
                this.buffer = this.buffer.slice(-512); // Keep last 512 bytes
            }

            // 🔄 Loop Protection: حداکثر 10 packet در هر بار
            let loopCount = 0;
            const MAX_LOOPS = 10;

            // Loop to find valid packets in the buffer
            while (this.buffer.length > 0 && loopCount < MAX_LOOPS) {
                loopCount++;

                // 1. Check for Config Response Header (FD FC FB FA)
                const configIdx = this.buffer.indexOf(Buffer.from([0xfd, 0xfc, 0xfb, 0xfa]));

                // 2. Check for Data Header (F4 F3 F2 F1)
                const dataIdx = this.buffer.indexOf(Buffer.from([0xf4, 0xf3, 0xf2, 0xf1]));

                // If no headers found, discard noise but keep last few bytes just in case
                if (configIdx === -1 && dataIdx === -1) {
                    if (this.buffer.length > 50) this.buffer = this.buffer.slice(-20);
                    break;
                }

                // Determine which packet comes first
                let type = "none";
                let startIdx = -1;

                if (configIdx !== -1 && (dataIdx === -1 || configIdx < dataIdx)) {
                    type = "config";
                    startIdx = configIdx;
                } else if (dataIdx !== -1) {
                    type = "data";
                    startIdx = dataIdx;
                }

                // Process the packet found
                if (startIdx > 0) {
                    // Discard bytes before the header (noise)
                    this.buffer = this.buffer.slice(startIdx);
                    startIdx = 0;
                }

                // Check if we have enough length for Length Bytes
                if (this.buffer.length < 6) break;

                // 🛡️ Safe read با try-catch
                let payloadLen, totalLen;
                try {
                    // Read Payload Length
                    payloadLen = this.buffer.readUInt16LE(4);
                    
                    // 🛡️ Sanity check: payloadLen نباید خیلی بزرگ باشه
                    if (payloadLen > 1024) {
                        this.log(`⚠️ Invalid payload length: ${payloadLen}, skipping packet`, 'warning');
                        this.buffer = this.buffer.slice(6); // Skip header
                        continue;
                    }
                    
                    totalLen = 4 + 2 + payloadLen + 4; // Header + Len + Payload + Footer
                } catch (error) {
                    this.log(`❌ Error reading payload length: ${error?.message}`, 'error');
                    this.buffer = this.buffer.slice(1); // Skip 1 byte and try again
                    continue;
                }

                if (this.buffer.length < totalLen) break; // Wait for more data

                // Extract the full packet
                const packet = this.buffer.slice(0, totalLen);

                // 🛡️ Handle packet با error handling
                try {
                    this.stats.packetsReceived += 1;
                    this.stats.lastPacketTime = Date.now();
                    
                    if (type === "config") {
                        this.stats.configPackets += 1;
                        this.parseConfigPacket(packet);
                    } else {
                        this.stats.dataPackets += 1;
                        this.parseDataPacket(packet);
                    }
                } catch (parseError) {
                    this.stats.errors += 1;
                    this.log(`❌ Error parsing ${type} packet (Total errors: ${this.stats.errors}): ${parseError?.message}`, 'error');
                    // ادامه بده، packet رو دور بریز
                }

                // Remove processed packet from buffer
                this.buffer = this.buffer.slice(totalLen);
            }

            // 🛡️ اگر loop limit رسیدیم، warning بده
            if (loopCount >= MAX_LOOPS) {
                this.log(`⚠️ Max loop limit reached, deferring remaining packets`, 'warning');
            }

        } catch (error) {
            this.stats.errors += 1;
            this.log(`❌ CRITICAL error in handleIncomingData (Total errors: ${this.stats.errors}): ${error?.message}`, 'error');
            // در صورت خطای شدید، buffer رو reset کن
            this.buffer = Buffer.alloc(0);
        }
    }

    /**
     * 📊 دریافت آمار اتصال (برای debugging)
     */
    getStats() {
        const now = Date.now();
        const uptime = this.stats.connectionTime > 0 ? 
            Math.floor((now - this.stats.connectionTime) / 1000) : 0;
        const timeSinceLastPacket = this.stats.lastPacketTime > 0 ? 
            Math.floor((now - this.stats.lastPacketTime) / 1000) : 'N/A';

        return {
            ...this.stats,
            uptime: `${uptime}s`,
            timeSinceLastPacket: `${timeSinceLastPacket}s`,
            isConnected: this.isConnected,
            bufferSize: this.buffer.length,
        };
    }

    /**
     * 🔄 Reset کردن آمار
     */
    resetStats() {
        this.stats = {
            packetsReceived: 0,
            configPackets: 0,
            dataPackets: 0,
            errors: 0,
            lastPacketTime: 0,
            connectionTime: 0,
            reconnectCount: 0,
            bufferOverflows: 0,
        };
        this.log('Statistics reset', 'info');
    }

    parseConfigPacket(packet) {
        try {
            // 🛡️ Validate packet size
            if (!packet || packet.length < 10) {
                this.log('⚠️ Config packet too small', 'warning');
                return;
            }

            // Payload starts at index 6
            const payload = packet.slice(6, packet.length - 4);

            // Check if it's a Read Parameter Response (Command 0x61, ACK 0x01)
            // Structure: 61 01 00 00 AA [Data...]
            if (payload[0] === 0x61 && payload[1] === 0x01) {
                const aaIndex = payload.indexOf(0xaa);
                if (aaIndex !== -1 && aaIndex + 23 < payload.length) {
                    const data = payload.slice(aaIndex + 1);
                    
                    // 🛡️ Validate array bounds
                    if (data.length < 23) {
                        this.log('⚠️ Config data too short', 'warning');
                        return;
                    }
                    
                    // data[1] = Max Moving Gate
                    // data[2] = Max Static Gate
                    // Duration is at the end bytes
                    const maxGate = data[1];
                    const duration = data[21] + (data[22] << 8); // Little Endian

                    if (this.onConfigRead) {
                        this.onConfigRead({ maxGate, duration });
                    }
                    this.log(`Config Read: MaxGate=${maxGate}, Time=${duration}s`, "success");
                }
            }
        } catch (error) {
            this.log(`❌ Error in parseConfigPacket: ${error?.message}`, 'error');
        }
    }

    parseDataPacket(packet) {
        try {
            const now = Date.now();

            // Real-time updates (50ms throttling to prevent overload)
            if (now - this.lastUpdateTime < this.updateInterval) return;
            this.lastUpdateTime = now;

            // 🛡️ Validate packet size
            if (!packet || packet.length < 17) {
                this.log(`⚠️ Data packet too small: ${packet?.length}`, 'warning');
                return;
            }

            // Payload starts at index 6
            // Standard Packet: F4 F3 F2 F1 [Len] [Type] [Head AA] [State] [MoveDist...]
            // The 'payload' variable here refers to internal data after Length bytes
            // Index 8 in full packet is usually Target State
            const targetState = packet[8] || 0;
            const moveDist = (packet[9] || 0) + ((packet[10] || 0) << 8);
            const moveEnergy = packet[11] || 0;
            const staticDist = (packet[12] || 0) + ((packet[13] || 0) << 8);
            const staticEnergy = packet[14] || 0;
            const detectDist = (packet[15] || 0) + ((packet[16] || 0) << 8);

            // Determine Logic State
            let statusText = "Absent";
            let isPresence = false;

            if (targetState === 1) statusText = "Moving";
            if (targetState === 2) statusText = "Stationary";
            if (targetState === 3) statusText = "Moving & Stationary";

            if (targetState !== 0) isPresence = true;

            // Emit data to UI
            if (this.onDataUpdate) {
                try {
                    this.onDataUpdate({
                        isPresence,
                        statusText,
                        distance: detectDist,
                        hex: packet
                            .toString("hex")
                            .toUpperCase()
                            .match(/.{1,2}/g)
                            .join(" "),
                    });
                } catch (callbackError) {
                    this.log(`❌ Error in onDataUpdate callback: ${callbackError?.message}`, 'error');
                }
            }
        } catch (error) {
            this.log(`❌ Error in parseDataPacket: ${error?.message}`, 'error');
        }
    }

    log(msg, type = "info") {
        // Logging disabled - silent mode
        // if (this.onLog) this.onLog(msg, type);
    }
}

export default new RadarLogic();
